...
const TodoListZustand = () => {
  ...
  const { todos, addTodo, toggleTodo, deleteTodo, sortTodo, reverseTodo  } = useTodoStore();
    ...
    <div className="min-h-screen flex items-center justify-center">
      <Card variant="default" className={`w-full max-w-md ${todos.length < 5 ? "h-auto" : "h-110" } rounded-xl shadow-lg p-6 flex flex-col`}>
        ...
        <Card.Description className="flex gap-2 justify-center">
          <Button onClick={sortTodo} className="bg-indigo-500 hover:bg-indigo-600 text-white px-3 py-1 rounded-lg transition">
            Sort
          </Button>
          <Button onClick={reverseTodo} className="bg-purple-500 hover:bg-purple-600 text-white px-3 py-1 rounded-lg transition">
            Reverse
          </Button>
        </Card.Description>
