...
import { addTodo, deleteTodo, reverseTodos, sortTodos, toggleTodo } from "../Redux/features/todo/todoSlice";
    ...
    <div className="min-h-screen flex items-center justify-center">
      <Card variant="default" className={`w-full max-w-md ${todos.length < 5 ? "h-auto" : "h-110" } rounded-xl shadow-lg p-6 flex flex-col`}>
        ...
        <Card.Description className="flex gap-2 justify-center">
          <Button onClick={() => dispatch(sortTodos())} className="bg-indigo-500 hover:bg-indigo-600 text-white px-3 py-1 rounded-lg transition">
            Sort
          </Button>
          <Button onClick={() => dispatch(reverseTodos())} className="bg-purple-500 hover:bg-purple-600 text-white px-3 py-1 rounded-lg transition">
            Reverse
          </Button>
        </Card.Description>
            ...
            <ScrollShadow orientation="vertical" hideScrollBar className="h-full pr-2">
                <div ref={parent} className="space-y-2">
                    {filteredTodos.map((todo) => (
                        ...
                        `}
                        >
                        <Card.Title onClick={() => dispatch(toggleTodo(todo.id))} className={`cursor-pointer ${todo.completed ? "line-through text-gray-400" : ""}`}>
                            {todo.text}
                        </Card.Title>

                        <Button variant="outline" onClick={() => dispatch(deleteTodo(todo.id))} className="text-red-500 hover:text-red-700">
                            <CloseIcon />
                        </Button>
                        </Card.Description>
                    ))}
                    {filteredTodos.length === 0 && (
                        <Card.Description className="text-center text-gray-400 mt-4">No tasks here</Card.Description>
                    )}
                </div>
            </ScrollShadow>
        </Card.Content>
